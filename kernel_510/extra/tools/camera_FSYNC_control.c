...
#define SYSFS_GPIO_SYS_FILE_EXPORTED_TIME_IN_10MS 10




int gpio_is_exported(jetsonGPIO gpio)
{
	int fd = 0;
	int i;
	char buf[64] = {0};
...
int gpioExport ( jetsonGPIO gpio )
{
    ...
    for (i = 0; i < SYSFS_GPIO_SYS_FILE_EXPORTED_TIME_IN_10MS; i++)//等待直到文件在用户空间中实际可用
    {
...
int gpioSetDirection ( jetsonGPIO gpio, unsigned int out_flag )
{
    ...
    fileDescriptor = open("/sys/class/gpio/PQ.06/direction", O_WRONLY);
    if (fileDescriptor < 0) {
        char errorBuffer[128] ;
...
int gpioSetValue ( jetsonGPIO gpio, unsigned int value )
{
    ...
    fileDescriptor = open("/sys/class/gpio/PQ.06/value", O_WRONLY);
    if (fileDescriptor < 0) {
        char errorBuffer[128] ;
...
int gpioGetValue ( jetsonGPIO gpio, unsigned int *value)
{
    ...
    fileDescriptor = open("/sys/class/gpio/PQ.06/value", O_RDONLY);
    if (fileDescriptor < 0) {
        char errorBuffer[128] ;
...
int gpioSetEdge ( jetsonGPIO gpio, char *edge )
{
    ...
    fileDescriptor = open("/sys/class/gpio/PQ.06/edge", O_WRONLY);
    if (fileDescriptor < 0) {
        char errorBuffer[128] ;
...
int gpioActiveLow ( jetsonGPIO gpio, unsigned int value )
{
    ...
    fileDescriptor = open("/sys/class/gpio/PQ.06/active_low", O_WRONLY);
    if (fileDescriptor < 0) {
        char errorBuffer[128] ;
...
int main()
{
	...
	if(access("/sys/class/gpio/PQ.06/direction",F_OK)){  /* 判断GPIO387是否已经导出 */
		ret=gpioExport(454);    /* 导出GPIO454 */
		if(ret<0){
			printf("export gpio error!\n");
		}
	}
	ret=gpioSetDirection(454,1);  /* 设置GPIO454方向为输出 */
	if(ret<0){
		printf("set  gpio direction error!\n");
		//return -1;
	}
	ret=gpioSetValue(454,1);   /* 设置输出电平为高电平 */
	if(ret<0){
		printf("set  gpio value error!\n");
	}
	        ret=gpioSetValue(454,0); /* 设置输出电平为低电平 */
