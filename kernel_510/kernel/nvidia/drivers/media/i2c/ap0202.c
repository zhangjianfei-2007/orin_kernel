...
#include "ap0202_mode_tbls.h"

#define AP0202_DEFAULT_MODE	AP0202_MODE_1920X1080_30FPS

#define AP0202_DEFAULT_DATAFMT MEDIA_BUS_FMT_YUYV8_2X8

#define AP0202_MIN_FRAME_LENGTH (1125)
#define AP0202_MAX_FRAME_LENGTH (1125)
#define AP0202_DEFAULT_FRAME_LENGTH    (1125)

/* default image output width */
#define AP0202_DEFAULT_WIDTH    1920
/* default image output height */
#define AP0202_DEFAULT_HEIGHT    1080

/* default output clk frequency for camera */
#define AP0202_DEFAULT_CLK_FREQ    24000000
...
struct ap0202_mutex_lock {
	int mutex_cnt;
	struct mutex stream_lock;
};
static struct ap0202_mutex_lock ap0202_mutex_lock[DESER_TOTAL];
#endif
...
static const struct v4l2_ctrl_ops ap0202_ctrl_ops = {
...
/* Do not change the name field for the controls! */
	{
		...
		.min = AP0202_MIN_FRAME_LENGTH,
		.max = AP0202_MAX_FRAME_LENGTH,
		.def = AP0202_DEFAULT_FRAME_LENGTH,
		.step = 1,
	},
};
...
static inline int ap0202_read_reg(struct camera_common_data *s_data,
				u16 addr, u8 *val)
{
	struct ap0202 *priv = (struct ap0202 *)s_data->priv;
	int err = 0;
	u32 reg_val = 0;
...
static int ap0202_write_reg(struct camera_common_data *s_data,
				u16 addr, u8 val)
{
	int err;
	struct ap0202 *priv = (struct ap0202 *)s_data->priv;
...
/*
int ap0202_write_reg2(struct ap0202 *priv, u8 addr, u8 val)
{
	int err;
	struct device *dev;
...
#if 0
static int ap0202_write_table(struct ap0202 *priv,
				const ap0202_reg table[])
{
	return regmap_util_write_table_8(priv->regmap,
					 table,
					 NULL, 0,
					 AP0202_TABLE_WAIT_MS,
					 AP0202_TABLE_END);
}
#endif
...
static int ap0202_gmsl_serdes_setup(struct ap0202 *priv)
{
	int err = 0;
	struct device *dev;
...
#if 0
static int SerDes_setup_pX(struct ap0202 *priv, int index)
{
//	int err = 0;
	int offset = 0;
	u8 value = 0;
...
//step 4: 96705
	...
	if(ser_linked_map[priv->dser_num] & 0x01) {
		...
		ap0202_write_reg2(priv,0x04, 0x87);
...
static void ap0202_gmsl_serdes_reset(struct ap0202 *priv)
{
	mutex_lock(&serdes_lock__);
...
static int ap0202_power_on(struct camera_common_data *s_data)
{
	int err = 0;
	struct ap0202 *priv = (struct ap0202 *)s_data->priv;
...
static int ap0202_power_off(struct camera_common_data *s_data)
{
	int err = 0;
	struct ap0202 *priv = (struct ap0202 *)s_data->priv;
	struct camera_common_power_rail *pw = &priv->power;
...
static int ap0202_power_get(struct ap0202 *priv)
{
	struct camera_common_power_rail *pw = &priv->power;
...
static int ap0202_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	mutex_lock(&ap0202_mutex_lock[priv->dser_num].stream_lock);
	...
	#ifdef __MUTEX_LOCK__
		mutex_unlock(&ap0202_mutex_lock[priv->dser_num].stream_lock);
...
static int ap0202_g_input_status(struct v4l2_subdev *sd, u32 *status)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct camera_common_data *s_data = to_camera_common_data(&client->dev);
	struct ap0202 *priv = (struct ap0202 *)s_data->priv;
...
static struct v4l2_subdev_video_ops ap0202_subdev_video_ops = {
	.s_stream	= ap0202_s_stream,
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0)
	.g_mbus_config = camera_common_g_mbus_config,
#endif
	.g_input_status = ap0202_g_input_status,
};

static struct v4l2_subdev_core_ops ap0202_subdev_core_ops = {
	.s_power	= camera_common_s_power,
};

static int ap0202_get_fmt(struct v4l2_subdev *sd,
...
static int ap0202_set_fmt(struct v4l2_subdev *sd,
		struct v4l2_subdev_pad_config *cfg,
	struct v4l2_subdev_format *format)
{
	int ret;
...
static struct v4l2_subdev_pad_ops ap0202_subdev_pad_ops = {
	.set_fmt = ap0202_set_fmt,
	.get_fmt = ap0202_get_fmt,
...
static struct v4l2_subdev_ops ap0202_subdev_ops = {
	.core	= &ap0202_subdev_core_ops,
	.video	= &ap0202_subdev_video_ops,
	.pad = &ap0202_subdev_pad_ops,
};

const struct of_device_id ap0202_of_match[] = {
	{ .compatible = "nvidia,ap0202",},
	{ },
};

static struct camera_common_sensor_ops ap0202_common_ops = {
	.power_on = ap0202_power_on,
	.power_off = ap0202_power_off,
	.write_reg = ap0202_write_reg,
	.read_reg = ap0202_read_reg,
};

static int ap0202_s_ctrl(struct v4l2_ctrl *ctrl)
{
	struct ap0202 *priv =
		container_of(ctrl->handler, struct ap0202, ctrl_handler);
	int err = 0;
...
static int ap0202_ctrls_init(struct ap0202 *priv)
{
...
static struct camera_common_pdata *ap0202_parse_dt(struct ap0202 *priv,
				...
				struct camera_common_data *s_data)
{
	...
	match = of_match_device(ap0202_of_match, &client->dev);
	if (!match) {
...
static int ap0202_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
...
static const struct media_entity_operations ap0202_media_ops = {
			...
			const struct i2c_device_id *id)
{
	...
	priv = devm_kzalloc(&client->dev,
			    sizeof(struct ap0202) + sizeof(struct v4l2_ctrl *) *
	...
	common_data->ops = &ap0202_common_ops;
	common_data->ctrl_handler = &priv->ctrl_handler;
	common_data->dev        = &client->dev;
	common_data->frmfmt = &ap0202_frmfmt[0];
	common_data->colorfmt = camera_common_find_datafmt(
					  AP0202_DEFAULT_DATAFMT);
	common_data->power = &priv->power;
	common_data->ctrls = priv->ctrls;
	common_data->priv = (void *)priv;
	common_data->numctrls = ARRAY_SIZE(ctrl_config_list);
	common_data->numfmts = ARRAY_SIZE(ap0202_frmfmt);
	common_data->def_mode = AP0202_DEFAULT_MODE;
	common_data->def_width = AP0202_DEFAULT_WIDTH;
	common_data->def_height = AP0202_DEFAULT_HEIGHT;
	common_data->fmt_width = common_data->def_width;
	common_data->fmt_height = common_data->def_height;
	common_data->def_clk_freq = AP0202_DEFAULT_CLK_FREQ;

	priv->pdata = ap0202_parse_dt(priv, client, common_data);
	if (!priv->pdata) {
...
*/
	 ...
	 */
	#if 1
	err = ap0202_gmsl_serdes_setup(priv);
	if (err) {
		dev_err(&client->dev,
	...
	err = camera_common_initialize(common_data, "ap0202");
	if (err) {
		dev_err(&client->dev, "Failed to initialize ap0202.\n");
		return err;
	}

	v4l2_i2c_subdev_init(priv->subdev, client, &ap0202_subdev_ops);

	err = ap0202_ctrls_init(priv);
	if (err)
		return err;

	priv->subdev->internal_ops = &ap0202_subdev_internal_ops;
	priv->subdev->flags |= V4L2_SUBDEV_FL_HAS_DEVNODE |
		     V4L2_SUBDEV_FL_HAS_EVENTS;

#if defined(CONFIG_MEDIA_CONTROLLER)
	priv->pad.flags = MEDIA_PAD_FL_SOURCE;
	priv->subdev->entity.ops = &ap0202_media_ops;
...
#ifdef __MUTEX_LOCK__
	if (ap0202_mutex_lock[priv->dser_num].mutex_cnt == 0){
		dev_info(&client->dev, " %s: mutex_init. \n", __func__);
		mutex_init(&ap0202_mutex_lock[priv->dser_num].stream_lock);
		ap0202_mutex_lock[priv->dser_num].mutex_cnt++;
	}
#endif

	dev_info(&client->dev, "Detected AP0202 sensor\n");
...
#ifdef __MUTEX_LOCK__
	if (ap0202_mutex_lock[priv->dser_num].mutex_cnt){
		dev_info(&client->dev, " %s: mutex_destroy. \n", __func__);
		mutex_destroy(&ap0202_mutex_lock[priv->dser_num].stream_lock);
		ap0202_mutex_lock[priv->dser_num].mutex_cnt--;
	}
...
static int __init ap0202_init(void)
{
	mutex_init(&serdes_lock__);

	return i2c_add_driver(&ap0202_i2c_driver);
}

static void __exit ap0202_exit(void)
{
	mutex_destroy(&serdes_lock__);

	i2c_del_driver(&ap0202_i2c_driver);
}

module_init(ap0202_init);
module_exit(ap0202_exit);

MODULE_DESCRIPTION("Media Controller driver for AP0202 yuv camera");
