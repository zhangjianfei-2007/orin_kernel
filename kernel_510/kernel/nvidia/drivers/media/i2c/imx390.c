...
#define IMX390_FSYNC_ADDR		0x3650
...
struct camera_common_data *sen_imx390_listp[12]={NULL};
EXPORT_SYMBOL(sen_imx390_listp);
int n_imx390_dev=0;
EXPORT_SYMBOL(n_imx390_dev);
...
int imx390_read_reg(struct camera_common_data *s_data,
				u16 addr, u8 *val)
{
	int err = 0;
	u32 reg_val = 0;
...
EXPORT_SYMBOL(imx390_read_reg);

int imx390_write_reg(struct camera_common_data *s_data,
				u16 addr, u8 val)
{
	int err;
...
EXPORT_SYMBOL(imx390_write_reg);
...
static int imx390_set_mode(struct tegracam_device *tc_dev)
{
	...
	if (priv->sensor_model == SENSOR_IMX390_RAW_SG){
		max9295_write_reg(priv->ser_dev, 0x02BE,0x10);	//MFP0/GPIO0
		msleep(100);
	}

	err = imx390_write_table(priv, mode_table[s_data->mode_prop_idx]);
	if (priv->g_ctx.frame_sync_en)
		imx390_write_reg(s_data, IMX390_FSYNC_ADDR, 0x01);
...
static int imx390_board_setup(struct imx390 *priv)
{
	...
	if (!strcmp(str_value, "imx390-BL")) {
		priv->sensor_model = SENSOR_IMX390_RAW_BL;
	} else if (!strcmp(str_value, "imx390-SG")) {
		priv->sensor_model = SENSOR_IMX390_RAW_SG;
	} else {
			...
			const struct i2c_device_id *id)
{
	...
	if(n_imx390_dev < 12){
		sen_imx390_listp[n_imx390_dev]=priv->s_data;
		n_imx390_dev++;
	}
