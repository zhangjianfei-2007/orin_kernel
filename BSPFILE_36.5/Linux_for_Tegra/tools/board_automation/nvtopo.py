...
class nv_topo_target:
    ...
    def __init__(self, device):
        ...
        io_list = {
                ...
                "GPIO_PIN12" : NV_TOPO_PIN["PIN12"],
                "GPIO_PIN14" : NV_TOPO_PIN["PIN14"],
                }
    ...
    def target_reset(self, delay=1):
        self.push_button("SYS_RESET", delay)


    def pre_smcu_flash(self):
        #recovery down
        nv_topo_set_gpio_value(self.device, self.IOs["GPIO_FRC_REC_N"], NV_TOPO_GPIO_VALUE["LOW"])
        #reset target
        self.target_reset(1)
        sleep_progress(1)
        #Putting SMCU in flash mode
        nv_topo_set_gpio_config(self.device, self.IOs["GPIO_PIN12"], NV_TOPO_GPIO_CONFIG["OUT_OD"], NV_TOPO_GPIO_INIT_STATE["LOW"] )
        nv_topo_set_gpio_value(self.device, self.IOs["GPIO_PIN12"], NV_TOPO_GPIO_VALUE["LOW"] )
        #Power cycle target sice aurix is blank
        nv_topo_set_gpio_config(self.device, self.IOs["GPIO_PIN14"], NV_TOPO_GPIO_CONFIG["OUT_OD"], NV_TOPO_GPIO_INIT_STATE["LOW"] )
        nv_topo_set_gpio_value(self.device, self.IOs["GPIO_PIN14"], NV_TOPO_GPIO_VALUE["LOW"] )
        sleep_progress(1)
        nv_topo_set_gpio_value(self.device, self.IOs["GPIO_PIN14"], NV_TOPO_GPIO_VALUE["HIGH"] )
        nv_topo_set_gpio_config(self.device, self.IOs["GPIO_PIN14"], NV_TOPO_GPIO_CONFIG["IN"], NV_TOPO_GPIO_INIT_STATE["HIGH"] )
        sleep_progress(1)

    def post_smcu_flash(self):
        #recovery down
        nv_topo_set_gpio_value(self.device, self.IOs["GPIO_FRC_REC_N"], NV_TOPO_GPIO_VALUE["HIGH"])
        #reset target
        self.target_reset(1)
        sleep_progress(1)
        #Starting target
        nv_topo_set_gpio_value(self.device, self.IOs["GPIO_PIN12"], NV_TOPO_GPIO_VALUE["HIGH"] )
        nv_topo_set_gpio_config(self.device, self.IOs["GPIO_PIN12"], NV_TOPO_GPIO_CONFIG["IN"], NV_TOPO_GPIO_INIT_STATE["HIGH"] )
...
class nv_topo:
    ...
    @open_close_ctx
    def reset_fw_normal(self):
        self.nv_topo_target.reset_fw(NV_TOPO_RESET_TYPE["NORMAL"])
    ...
    @open_close_ctx
    def pre_smcu_flash(self):
        self.nv_topo_target.pre_smcu_flash()

    @open_close_ctx
    def post_smcu_flash(self):
        self.nv_topo_target.post_smcu_flash()
