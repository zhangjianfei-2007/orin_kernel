...
- This tool does not support size discovery for internal emmc/sdcard. Therefore,
  ...
  bootloader/generic/cfg if the default "num_sectors" is incompatible. You must
...
For a device with internal QSPI and external NVMe: For the first or only NVMe:
sudo ./tools/kernel_flash/l4t_initrd_flash.sh --external-device nvme0n1p1 \
      -c tools/kernel_flash/flash_l4t_external.xml \
      -p "-c bootloader/generic/cfg/flash_t234_qspi.xml --no-systemimg" --network usb0 \
      <board> external

For a device with internal QSPI and external NVMe: For the second NVMe:
