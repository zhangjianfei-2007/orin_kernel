...
def __check_required_optional(desc, path, items, required, optional):
    required_patterns = [re.compile('^{}$'.format(x)) for x in required]
    optional_patterns = [re.compile('^{}$'.format(x)) for x in optional]

    msg = "Node '{}' is missing required {} '{}'"
    for (patt_text, pattern) in zip(required, required_patterns):
        in_items = any([pattern.match(x) for x in items])
        assert in_items, msg.format(path, desc, patt_text)

    msg = "Node '{}' has unknown {} '{}'"
    for name in items:
        in_required = any([x.match(name) for x in required_patterns])
        in_optional = any([x.match(name) for x in optional_patterns])
        assert in_required or in_optional, msg.format(path, desc, name)

def __check_props(node, required=[], optional=[]):
    __check_required_optional("property", node.path(), node.props, required, optional)

def __check_children(node, required=[], optional=[]):
    __check_required_optional("child node", node.path(), node.children, required, optional)
...
def __check_isense(node):
    __check_props(node)
    __check_children(node,
            required=['channel-map', 'icl-map', 'adc', 'icl', 'vidpwm', 'ovr-m(@.+)?'],
            optional=['compound-map'])

def __check_isense_channel_map(node):
    __check_props(node)

def __check_isense_channel_map_node(node):
    __check_props(node,
            required=['sensor-id', 'channel-index', 'scale-factor'],
            optional=['offset-sensor-id'])
    __check_u32(node, 'sensor-id')
    __check_u32(node, 'channel-index')
    __check_float(node, 'scale-factor')
    __check_u32(node, 'offset-sensor-id',
                required=False)
    __check_children(node)

def __check_isense_compound_map(node):
    __check_props(node)

def __check_isense_compound_map_node(node):
    __check_props(node,
            required=['output-id', 'input-ids'])
    __check_u32(node, 'output-id')
    __check_u32(node, 'input-ids', func=lambda x: len(x) >= 1 and len(x) <= 4)
    __check_children(node)

def __check_isense_icl_map(node):
    __check_props(node)

def __check_isense_icl_map_node(node):
    __check_props(node,
            required=['controller-id', 'icl-index', 'scale-factor'])
    __check_u32(node, 'controller-id')
    __check_u32(node, 'icl-index')
    __check_float(node, 'scale-factor')
    __check_children(node)

def __check_isense_adc(node):
    __check_props(node,
            required=['raw-registers', 'num-samples'])
    __check_u32(node, 'raw-registers',
            func=lambda x: len(x) == 24)
    __check_u32(node, 'num-samples')
    __check_children(node)

def __check_isense_icl(node):
    __check_props(node,
            required=['raw-registers'])
    __check_u32(node, 'raw-registers',
            func=lambda x: len(x) == 84)
    __check_children(node)

def __check_isense_vidpwm(node):
    __check_props(node,
            required=['raw-registers'])
    __check_u32(node, 'raw-registers',
            func=lambda x: len(x) == 18)
    __check_children(node)

def __check_isense_ovrm(node):
    __check_props(node,
            required=['i2c-address'],
            optional=['allow-nack'])
    __check_children(node, required=['i2c-data'])

def __check_isense_ovrm_i2c_data(node):
    for name in node.props:
        __check_u8(node, name,
                func=lambda x: len(x) <= 500)
    __check_children(node)
...
def __check_regulator_ovr(node):
    ...
    __check_props(node,
            ...
            optional=['pwm-rate', 'enable-gpio', 'linux,phandle', 'phandle', 'vboot'])
...
def __check_regulator_da9092(node):
    # BPMPDTB-REGULATORS-50
    __check_props(node,
            required=['compatible', 'slave-address', 'channel'],
            optional=['enable-gpio', 'vsel-gpio', 'linux,phandle', 'phandle'])
    # BPMPDTB-REGULATORS-51
    __check_children(node)

    # BPMPDTB-REGULATORS-52
    assert node.props['compatible'] == b'da9092\x00'
    # BPMPDTB-REGULATORS-53
    __check_u32(node, 'slave-address',
                func=lambda x: (len(x) == 1 and
                                x[0] in range(0, 128)))
    # BPMPDTB-REGULATORS-54
    __check_u32(node, 'channel',
                func=lambda x: (len(x) == 1 and
                                x[0] in range(1, 3)))
    # BPMPDTB-REGULATORS-55
    __check_u32(node, 'enable-gpio', required=False)
    # BPMPDTB-REGULATORS-56
    __check_u32(node, 'vsel-gpio', required=False)
    # BPMPDTB-REGULATORS-57
    __check_u32(node, 'linux,phandle', required=False)
    # BPMPDTB-REGULATORS-58
    __check_u32(node, 'phandle', required=False)
...
def __check_regulator_dev(node):
    regulator_devs = {
            ...
            b'da9092\x00': __check_regulator_da9092,
            }
...
def __main():
    ...
    root_rules = {
        ...
        'th500':
        FdtRule('', __check_root, subrules=[
            FdtRule('isense', __check_isense, subrules=[
                FdtRule('channel-map', __check_isense_channel_map, subrules=[
                    FdtRule('.+', __check_isense_channel_map_node),
                ]),
                FdtRule('compound-map', __check_isense_compound_map, subrules=[
                    FdtRule('.+', __check_isense_compound_map_node),
                ]),
                FdtRule('icl-map', __check_isense_icl_map, subrules=[
                    FdtRule('.+', __check_isense_icl_map_node),
                ]),
                FdtRule('adc', __check_isense_adc),
                FdtRule('icl', __check_isense_icl),
                FdtRule('vidpwm', __check_isense_vidpwm),
                FdtRule('ovr-m', __check_isense_ovrm, subrules=[
                    FdtRule('i2c-data', __check_isense_ovrm_i2c_data),
                ]),
            ]),
            FdtRule('regulators', __check_regulators, subrules=[
                FdtRule('.+', __check_regulators_node),
    ...
    parser.add_argument('-o', '--output', type=argparse.FileType('wb'),
                        default=sys.stdout,
                        help='Output file (default stdout, deprecated behaviour)')
    ...
    try:
        ...
        if outfile.fileno() != sys.stdout.fileno() and os.path.isfile(outfile.name):
            outfile.close()
