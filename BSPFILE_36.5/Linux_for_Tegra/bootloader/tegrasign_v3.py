...
def extract_key(p_key, keyfilename, internal):
    ...
    # Handle HSM route next
        ...
        # This means key type is already found so can return
        if p_key.hsm.type == p_key.mode:
            return 1
        ...
        elif p_key.hsm.type == KeyType.PKC:
            p_key.mode = NvTegraSign_PKC
            if internal["--pubkeyhash"]:
                return is_PKC_pubkey(keyfilename, p_key, pk, mont) \
                    or is_PKC_key(keyfilename, p_key, pk, mont)
            return 1

        elif p_key.hsm.type == KeyType.ECC:
            p_key.mode = NvTegraSign_ECC
            if internal["--pubkeyhash"]:
                return is_ECC_key(keyfilename, p_key, pk)
            return 1

        elif p_key.hsm.type == KeyType.ED25519:
            p_key.mode = NvTegraSign_ED25519
            p_key.keysize = ED25519_SIG_SIZE
            if internal["--pubkeyhash"]:
                return is_ED25519_key(keyfilename, p_key, pk)
            return 1
        ...
        if is_PKC_pubkey(keyfilename, p_key, pk, mont) is True \
            or is_PKC_key(keyfilename, p_key, pk, mont) is True:
           p_key.mode = NvTegraSign_PKC
           p_key.filename = keyfilename
           return 1
        ...
        if internal["--key"] or internal["--hsm"] or is_kdf_file:
            ...
            if (type(internal["--iv"]) == str):
                if (internal["--iv"] == 'random'):
                    internal["--iv"] = random_gen(IV_SIZE)
                ...
                if internal["--kdf"]:
                    ...
                    if (kdf_arg_len == 1):
                        ...
                        with open(tag_file, 'wb') as f:
                            # Only write iv if not metablob-enable and is stage2
                            if p_keylist[0].block_size == '0' and p_keylist[0].kdf.flag == DerKey.SBK_PT:
                                f.write(kdf_list[KdfArg.IV][:])
