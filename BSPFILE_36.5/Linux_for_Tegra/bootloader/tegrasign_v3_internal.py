...
def sign_files_in_listmeta(p_key, internal):
    ...
    for line in lines:
        ...
        if sig_data == None or isinstance(sig_data, str):
            ...
            return exit_routine()
        if i*len(sig_data) + len(sig_data) > len(dest):
            info_print('Cannot continue as total size %d is smaller than requested %d' \
                %(len(dest), i*len(sig_data) + len(sig_data)))
            return exit_routine()
        dest[i*len(sig_data):i*len(sig_data)+len(sig_data)] = sig_data[:]
        i = i +1
    if count != i:
...
def is_ECC_key(keyfilename, p_key, pkh):
    if is_hsm():
        get_ecc_key_hsm = import_function('get_ecc_key_hsm')
        ecc_key_size = get_ecc_key_hsm(pkh)
        if ecc_key_size == 521:
            p_key.keysize = NV_ECC521_SIG_STRUCT_SIZE
            return True
        elif ecc_key_size == 256:
...
def create_keystore(params, p_key, create_dk = False, user_kdk = False, dk_ctx = None):
    ...
    for dk_id in dk_list:
        ...
        if create_dk == True:
            der_list[dk_id] = [p_key.kdf.label.get_strbuf(), p_key.kdf.context.get_strbuf(), last_kdk, dk_id] # SBK_MB1_KDK, label, SBK_MB1_DK,
        ...
        while int_kdk in ['SBK_NVMB_KDK', 'SBK_TZ_KDK', 'SBK_GP_TOSB_KDK', 'SBK_GP_KDK']:
            if int_kdk == 'SBK_NVMB_KDK':
                label = p_key.kdf.bl_label.get_strbuf()
            ...
            der_list[last_kdk] = [label, '', int_kdk, last_kdk]
        ...
        for key in reversed(der_list):
            ...
            if oem_hsm_kdf(kdk, vals[LBL], vals[CTX], unq_id) == False: # i.e. SBK, label, SBK_NVMB_KDK
...
def do_derive_hmacsha(p_key):
    if is_hsm():
        ...
        if oem_hsm_kdf(kdk, label, context, unq_id) == False: # i.e. FSKP_KDK, label, context, FKP_AK_5c2f4c33845e154b0250d1196fe0cfa9
                raise tegrasign_exception('Can not derive %s' % (unq_id))
