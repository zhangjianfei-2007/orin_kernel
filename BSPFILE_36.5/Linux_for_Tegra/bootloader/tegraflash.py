...
def usage():
    ...
    '   --coldboot_pt_layout : Cold boot Partition layout configuration file',
    '   --rcmboot_pt_layout : RCM Boot partition layout configuration file',
    '   --coldboot_bct_cfg: Cold boot BCT configuration',
    '   --rcmboot_bct_cfg: RCM boot BCT configuration',
    ...
    '   --dry_run       : Dry run the tegraflash command',
...
class tegraflashcmds(cmd.Cmd):
    ...
    def chip_parser(self):
        ...
        chip_major = int(exports['--chip_major'], 0)
        if chip_id == 0x23:
    ...
    def do_bmcflash(self, param):
        ...
        try:
            self.chip_inst.tegraflash_bmcflash(args)

        except tegraflash_exception as e:
            print('Error: '+ e.value)
            tegraflash_err(1)

    def help_bmcflash(self):
        print('\n'.join([
        ' ',
        '------------------------------------------------------',
        '  Usage: bmcflash <pldm_package>',
        '------------------------------------------------------',
        ' ',
        ]))
    ...
    def do_build(self, params):
       ...
       try:
           self.chip_inst.tegraflash_build(exports)
...
if __name__ == '__main__':
    ...
    if exports['--rcmboot_pt_layout'] is not None:
        exports['--rcmboot_pt_layout'] = tegraflash_update_img_path(exports['--rcmboot_pt_layout'], exports['--image_dirs'])

    if exports['--coldboot_pt_layout'] is not None:
        exports['--coldboot_pt_layout'] = tegraflash_update_img_path(exports['--coldboot_pt_layout'], exports['--image_dirs'])
