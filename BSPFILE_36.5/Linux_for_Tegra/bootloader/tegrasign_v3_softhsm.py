...
def hsm_get_pkc_key_type():
    try:
        ...
        with token.open(user_pin=hsm_user_pin) as session:
            try:
                key = session.get_key(label=KeyType.PKC,
                                    object_class=pkcs11.ObjectClass.PRIVATE_KEY)
                # Get key type attribute
                key_type = key[pkcs11.Attribute.KEY_TYPE]

                if key_type == pkcs11.KeyType.RSA:
                    # For RSA, check the key size to determine if it's RSA3072
                    if pkcs11.Attribute.MODULUS_BITS in key:
                        key_size = key[pkcs11.Attribute.MODULUS_BITS]
                        if key_size == 3072:
                            return "RSA"
...
@param[in] pub_key_file Optional file path to save extracted ECC public key in OpenSSL format
@retval 521 if the key is ECDSA P-521 curve
@retval 256 if the key is ECDSA P-256 curve
@retval 0 if any other curve type or error
'''
def get_ecc_key_hsm(pub_key_file):
    # input validation
    ...
    try:
        ...
        with token.open(user_pin=hsm_user_pin) as session:
            try:
                # get the private key
                private_key = session.get_key(label=KeyType.PKC,
                                            object_class=pkcs11.ObjectClass.PRIVATE_KEY)
                ...
                info_print(f'[HSM] Detected {curve_name} ECC key')

                # extract the public key
                try:
                    pub_key = session.get_key(label=KeyType.PKC,
                                            object_class=pkcs11.ObjectClass.PUBLIC_KEY)
                    ...
                    try:
                        ...
                        info_print(f'[HSM] Successfully extracted {curve_name} public key to {pub_key_file}')
