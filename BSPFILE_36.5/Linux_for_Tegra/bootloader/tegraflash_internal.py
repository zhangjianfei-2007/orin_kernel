...
tegraflash_binaries_v2 = { 'tegrabct':'tegrabct_v2', 'tegrahost':'tegrahost_v2', 'tegrasign':'tegrasign_v3.py', 'tegrarcm':'tegrarcm_v2', 'tegradevflash':'tegradevflash_v2', 'tegraparser':'tegraparser_v2', 'dtc':'dtc', 'cpp':'cpp', 'nvgrblobgen': 'nvgrblobgen'}
...
class grBlob():
    ...
    def getCSVfile(self, path, bootType, chipName):
        ...
        for f in csvDirectoryFiles:
            if(bootType.upper() in f and chipName.upper() in f and ".csv" in f):
                return f
        #Could not find it
        return None

    def GenerateGrBlobs(self, p_NvGrCsvPath, s_ChipName):
        f_CsvName = ""
        l_NvGrBlobGenCmd = exec_file(self.f_NvGrBlobGen)

        info_print("Generating GR Blobs")
        for grType in ['MB1', 'MB2']:

            f_CsvName = self.getCSVfile(p_NvGrCsvPath, grType, s_ChipName)

            if(f_CsvName == None):
                info_print("Error: CSV does not exist")
                return 1

            f_CsvName = os.path.abspath(os.path.join(p_NvGrCsvPath, f_CsvName))
            csvarg = "--" + grType.lower() + "csv"
            l_NvGrBlobGenCmd.extend([csvarg, f_CsvName])

        l_NvGrBlobGenCmd.extend(["--outdir", self.OutDir])
        run_command(l_NvGrBlobGenCmd)

        info_print("Successfully generated GR blobs")

    def AppendBlobToBin(self, BlobImage, BinImage, p_TempDumpPath):
        if(os.path.isfile(BinImage) == False):
            info_print("File not found " + BinImage, nverror.NvError_FileNotFound)
        elif(os.path.isdir(p_TempDumpPath) == False):
            info_print("Invalid path " + p_TempDumpPath, nverror.NvError_FileNotFound)

        info_print("Appending Blob = " + os.path.join(p_TempDumpPath,BlobImage) + " to binary = " + BinImage)
        try:
            ...
            info_print(msg, nverror.NvError_FileOperationFailed)

        return BinImage

    def AppendBlBlobs(self, mb1_bin, mb2_bin, s_ChipName):
        mb1_with_grblob = os.path.splitext(mb1_bin)[0] + '_with_' + os.path.splitext(self.f_NvMbGrBlobImage)[0] + '.bin'
        shutil.copyfile(mb1_bin, mb1_with_grblob)

        info_print("Appending Blob for MB1")
        self.AppendBlobToBin(self.f_NvMbGrBlobImage, mb1_with_grblob, self.OutDir)

        mb2_with_grblob = os.path.splitext(mb2_bin)[0] + '_with_' + os.path.splitext(self.f_NvMbGrBlobImage)[0] + '.bin'
        shutil.copyfile(mb2_bin, mb2_with_grblob)

        info_print("Appending Blob for MB2")
        self.AppendBlobToBin(self.f_NvMbGrBlobImage, mb2_with_grblob, self.OutDir)

        return mb1_with_grblob, mb2_with_grblob
...
def run_command(cmd, enable_print=True):
    ...
    if '--dry_run' in values and (values["--dry_run"]):
        dry_run_cmd = ' '.join(cmd)
        if ('tegradevflash' in dry_run_cmd or 'tegrarcm' in dry_run_cmd):
            info_print('dry_run: Running command: ' + dry_run_cmd)
            return
...
def tegraflash_secureflash(args):
    ...
    if values['--bct_backup']:
        # Use "bct_backup.img" generated by "sign" command
        tegrabct_backup['--image']="bct_backup.img"
    tegraflash_flash_bct()
    info_print('Flashing completed\n')
...
def concat_file_4k(indtbfile, infilenames):
    # Copy input dtb file to temp file
    tmp_output_dtb_file = indtbfile + ".updated"
    shutil.copy(indtbfile, tmp_output_dtb_file)

    info_print('Concatenating ' + ','.join(infilenames) + ' to ' + tmp_output_dtb_file)
    for infilename in infilenames:
        with open(tmp_output_dtb_file, 'ab+') as outfile:
            with open(infilename, 'rb') as infile:
                cur = outfile.tell()
...
def tegraflash_concat_overlay_dtb():
    ...
    with open(values['--bldtb'], 'ab+') as outfile:
...
# Generate GR Blob and append to mb1 and mb2 binaries
def generate_gr_blob(chip_id, mb1_bin, mb2_bin):
    flash_path = os.getcwd()
    csv_path = os.getcwd()

    GrBlob = grBlob(flash_path)
    grBlob.GenerateGrBlobs(GrBlob, csv_path, chip_id)
    mb1_with_grblob, mb2_with_grblob = grBlob.AppendBlBlobs(GrBlob, mb1_bin, mb2_bin, chip_id)
...
def tegraflash_parse_partitionlayout():
    if int(values['--chip'], 0) in [0x23, 0x24]:
