...
class TFlashT23x_Base(object):
    ...
    def tegraflash_generate_br_bct(self, coldboot_bct):
        ...
        # Generate and update SHA digest for BR-BCT from u8_reserved_signed to u8_reserved_signed_encrypted.
        info_print('Generating SHA2 Hash')
        reserved_signed_encrypted_offset = 6848
        reserved_signed_offset = 5900
        length = 948
        sha_size = 64

        # Do the binary digest first
        with open(bct_file, 'rb+') as f:
            src = bytearray(f.read())
            sha_file_name = compute_sha('sha512', bct_file, reserved_signed_offset, length)
            # Append to end of binary
            with open(sha_file_name, 'rb') as sha_f:
                src[reserved_signed_encrypted_offset:reserved_signed_offset+sha_size] = bytearray(sha_f.read())
            f.seek(0)
            f.write(src)
    ...
    def tegraflash_generate_blob(self, sign_images, blob_filename, is_coldboot_mb2bct, mb2_is_fskp=False):
        ...
        images_to_sign = ['dce_fw', 'mts_mce', 'mb2_bootloader', 'fusebypass', 'mb2_applet',
                ...
                'tsec_fw', 'nvdec', 'xusb_fw', 'rce_fw', 'fsi_fw', 'fskp_bin', 'bpmp_ist', 'ccplex_ist', 'ist_ucode', 'pva_fw']
        ...
        for binary in bins:
            ...
            if tags[0] == 'dce_fw' and sign_images:
                kernel_dtb = self.get_dce_base_dtb_filename()
                if kernel_dtb is None:
                    ...
                    filename = dce_comb
            if tags[0] == 'fskp_bin' and values['--fuse_info'] is not None:
                fuse_info_xml = values['--fuse_info']
                fuse_info_bin = os.path.splitext(fuse_info_xml)[0] + '.bin'
    ...
    def tegraflash_oem_enc_fskp(self, in_file):
        ...
        if values['--chip_major'] == '0':
            FSKP_EK = '60b0a25a2b7bcc4d48e7ab3cda9c157fcbb260a4e5c8b5be24f665a1ed93cbd1'
        else:
            if values['--chip_minor'] == '67':
                FSKP_EK = '88f8a7bdd3aaead51370534b9bec9ffbaed1df68ae580aa6f243d18f992d56ac'
            else:
                FSKP_EK = '44d4c46f8f2e2e88a410152c96c9e55f356c2ca4854cc0ad5b8a68988a3242b8'
        fskp_ek = 'fskp_ek.key'
    ...
    def tegraflash_oem_sign_fskp(self, in_file):
        ...
        if values['--chip_major'] == '0':
            FSKP_AK = '82293353f48428a187b096c347a52b690673d1878ee00e67e784d0758bb3ccff'
        else:
            if values['--chip_minor'] == '67':
                FSKP_AK = '1996b10473aa24ab67276a629bcbf162130c17008895b67fa37b6a1f30bb0e32'
            else:
                FSKP_AK = 'cb2bf976fc97edb29b5fe2ab8b31a6884f1f06d41f5f5c4d59410c9d62781770'
        fskp_ak = 'fskp_ak.key'
    ...
    def tegraflash_generate_mem_bct(self, is_cold_boot_mb1_bct):
        if values['--sdram_config'] is None:
            info_print('Info: Skip generating mem_bct because sdram_config is not defined')
            return 1
        if bool(is_cold_boot_mb1_bct) == True:
    ...
    def tegraflash_concat_partition(self, partition_name, filename):
        ...
        if partition_name in cpu_bl_list:
            ...
            #    raise tegraflash_exception('Error: Could not find dce_bin: ' + dce_bin)
            #if not os.path.exists(kernel_dtb):
            #    raise tegraflash_exception('Error: Could not find kernel_dtb: ' + kernel_dtb)
    ...
    def tegraflash_dump(self, args, dump_args):
        ...
        if self.check_is_mb2applet():
            ...
            elif dump_args[0] == 'tnspec':
                self.tegraflash_dumpnct(dump_args[1:])
            elif dump_args[0] == 'try_custinfo':
                try:
                    self.tegraflash_dumpcustinfo(dump_args[1:])
                except:
                    info_print("Error: try getting custinfo fail. Moving on")
            else:
                 raise tegraflash_exception(dump_args[0] + " is not supported")
        else:
            info_print('Error: mb2 applet is not running\n')

    def tegraflash_dumpnct(self, dump_args):
        info_print('Dumping nct')
        out_file = tegraflash_abs_path(dump_args[0])
        command = self.exec_file('tegrarcm')
        command.extend(['--chip', values['--chip'], values['--chip_major']])
        command.extend(['--read', 'NCT', out_file])
        run_command(command)
    ...
    def tegraflash_sign(self, exports, args=None):
        ...
        images_to_sign = ['dce_fw', 'mts_mce',
                ...
                'mb2_applet', 'xusb_fw', 'rce_fw', 'fsi_fw', 'bpmp_ist',
                'ccplex_ist', 'ist_ucode', 'pva_fw']
        binaries = []
                ...
                if tags[0] in images_to_sign:
                    ...
                    # dce fw needs to concatenate with kernel dtb
                        ...
                        if kernel_dtb is None:
                            ...
                            tags[1] = self.concatenate_dcebin_kerneldtb(tags[1], kernel_dtb)
                    magic_id = self.tegraflash_get_magicid(tags[0])
                    if values['--encrypt_key'] is None:
                    ...
                    # flash.sh assumes a fixed name format, so create a copy since dce has name change
                    if tags[0] == 'dce_fw' and '_sigheader' in tags[1]:
                        new_name = tags[1].split('_sigheader')
                        org_name = os.path.splitext(org_name)
                        new_name = org_name[0] + '_sigheader' + new_name[1]
                        shutil.copyfile(tags[1], new_name) # Make a duplicate of final dce file
                        binaries.extend([new_name])
    ...
    def exec_file(self, name):
        ...
        if '--instance' in values and values['--instance'] is not None and name in supports_instance:
    ...
    def run_cpp_tool(self, input_file):
        ...
        command.extend(["-I", os.path.relpath(tegraflash_abs_path("generic/BCT"))])
