...
Examples:
    ...
    2. Build BUP image completely offline by providing board id, fab, fuse level
       and chip sku through environment variables:
        $ sudo FAB=B00 BOARDID=3701 BOARDSKU=0000 FUSELEVEL=fuselevel_production \\
            CHIP_SKU=\"00:00:00:D0\" ./build_l4t_bup.sh jetson-agx-orin-devkit mmcblk0p1
    ...
    4. Build PKC key signed images BUP:
        $ sudo FAB=B00 BOARDID=3701 BOARDSKU=0000 FUSELEVEL=fuselevel_production \\
            CHIP_SKU=\"00:00:00:D0\" ./build_l4t_bup.sh -s <pkc_key_file> \\
            jetson-agx-orin-devkit mmcblk0p1
    ...
    5. Build single image BUP offline by providing board SPEC:
        $ sudo FAB=000 BOARDID=3701 BOARDSKU=0004 BOARDREV= FUSELEVEL=1 CHIPREV= \\
            CHIP_SKU=\"00:00:00:D2\" ./build_l4t_bup.sh --single-image-bup A_xusb-fw \\
            jetson-agx-orin-devkit mmcblk0p1
    ...
    6. Build bootloader image BUP offline by providing board SPEC:
        $ sudo FAB=000 BOARDID=3767 BOARDSKU=0000 BOARDREV= FUSELEVEL=1 CHIPREV= \\
            CHIP_SKU=\"00:00:00:D3\" ./build_l4t_bup.sh --bup-type bl \\
...
"${BUP_DIR}/flash.sh" "--no-flash" "--sign" "--bup" "${@}"
exit "${?}"
