...
gen_param()
{
	...
	if [ "${bctarg}" != "" ] && [ "${mb1bctarg}" != "" ]; then
		...
		local binsimg_23=("dce_fw" "mts_mce" "mb2_bootloader" "fusebypass" "mb2_applet" "bootloader_dtb" "spe_fw" "bpmp_fw" "bpmp_fw_dtb" "psc_fw" "tos" "eks" "sce_fw" "ape_fw" "tsec_fw" "nvdec" "xusb_fw" "rce_fw" "fsi_fw" "fskp_bin" "bpmp_ist" "ccplex_ist" "ist_ucode" "kernel" "kernel_dtb");
		local na;
		local suf;
		...
		for (( i=0; i<${#binsimg_23[@]}; i++ )); do
			...
			if [ "${binsimg_23[$i]}" = "mb2_bootloader" ]; then
				bn="${na}_aligned_sigheader.${suf}.encrypt";
				...
				getidx "kernel_dtb" "" "${a[@]}";
				j=$?;
				# the suffix .dtb should be removed.
				dtbname=${a[$j]%%.*};
				bn="${na}_with_${dtbname}_sigheader.${suf}.encrypt";
				cp -f "${binsfilename}" "${bn}";
				cat "${a[$j]}" >> "${bn}";
			elif [ "${binsimg_23[$i]}" = "bpmp_fw_dtb" ]; then
				bn="${na}_with_odm_sigheader.${suf}.encrypt";
			elif [ "${binsimg_23[$i]}" = "kernel" ]; then
				bn="${na}.${suf}";
			elif [ "${binsimg_23[$i]}" = "kernel_dtb" ]; then
				bn="${na}.${suf}";
			else
				bn="${na}_sigheader.${suf}.encrypt";
			fi;
...
tar xjf "${FUSEBLOB}" "${BLDIRNAME}/${FUSECMD}"
...
if [ "${tgid}" = "0x19" ]; then
	tar xjf "${FUSEBLOB}" "${BLDIRNAME}/${fusecfg}";
	mv "${BLDIRNAME}/${fusecfg}" .;
	rmdir "${BLDIRNAME}";
fi;
...
EOF
	...
	pushd "${signdir}" > /dev/null 2>&1 || exit 1;
	cp fskp_t234_updated.bin fskp_t234_updated_aligned.bin
	tegrahost_v2 --chip 0x23 0 --align fskp_t234_updated_aligned.bin

	# generate fskp_t234_updated_aligned_sigheader.bin
	tegrahost_v2 --chip 0x23 0 --magicid MB2B --appendsigheader fskp_t234_updated_aligned.bin zerosbk

	# generate fskp_t234_updated_aligned_sigheader.bin_list.xml
	cat > fskp_t234_updated_aligned_sigheader.bin_list.xml << EOF
<?xml version="1.0"?>
<!-- Auto generated by tegrasign -->

<file_list mode="sbk"><file name="fskp_t234_updated_aligned_sigheader.bin" offset="4032" length="4160"><sbk encrypt="1" sign="1" encrypt_file="fskp_t234_updated_aligned_sigheader.bin.encrypt" hash="fskp_t234_updated_aligned_sigheader.bin.hash" /><pkc signature="fskp_t234_updated_aligned_sigheader.bin.sig" signed_file="fskp_t234_updated_aligned_sigheader.bin.signed" /><ec signature="fskp_t234_updated_aligned_sigheader.bin.sig" signed_file="fskp_t234_updated_aligned_sigheader.bin.signed" /><eddsa signature="fskp_t234_updated_aligned_sigheader.bin.sig" signed_file="fskp_t234_updated_aligned_sigheader.bin.signed" /><xmss signature="fskp_t234_updated_aligned_sigheader.bin.sig" signed_file="fskp_t234_updated_aligned_sigheader.bin.signed" /></file></file_list>
EOF

	# generate fskp_t234_updated_aligned_sigheader.bin.hash
	tegrasign_v3.py --key None --list fskp_t234_updated_aligned_sigheader.bin_list.xml --pubkeyhash pub_key.key --sha sha512

	# update sigheader
	tegrahost_v2 --chip 0x23 0 --updatesigheader fskp_t234_updated_aligned_sigheader.bin.encrypt fskp_t234_updated_aligned_sigheader.bin.hash zerosbk
	popd > /dev/null 2>&1 || exit 1;
fi;
